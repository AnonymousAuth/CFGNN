...
class CFGNN(nn.Module):
    def __init__(self, opt):
        ...
        self.node_lstm = nn.LSTM(self.hidden_dim, self.hidden_dim//2, bidirectional=True, batch_first=True)
                        #nn.RNN(self.hidden_dim, self.hidden_dim, bidirectional=False, batch_first=True)
        self.gate = nn.LSTMCell(self.hidden_dim, self.hidden_dim)
        self.back_gate = nn.LSTMCell(self.hidden_dim, self.hidden_dim)
